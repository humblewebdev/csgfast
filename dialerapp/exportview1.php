...
while ($bl = mysql_fetch_array($bucketlist)) {
			 $bucketid = $bl['id'];
			 $disptypes_arr = array("fa01","fa02","fa03","fa05","fa06","fa07","fa09","fr01","fr02","fr03","fr04","fr05","fr08");
			 ...
			 $disptypes_c = '"' . implode('","', $disptypes_arr_c) . '"';
			  ...
			  echo "<td>";
			  while($disp = mysql_fetch_array($bucket_link_list_att)){
			  
			  echo" 
			        <strong>Disp:</strong> {$disp['link_disp_type']}, <br>
			        <strong>Notes:</strong> {$disp['link_notes']}, <br>
					<strong>Agent:</strong> {$disp['link_action_by']}, <br>
					<strong>Time:</strong> {$disp['link_timestamp']} <br><br>";
			  }
			  echo "</td>";
			  echo "<td>";
