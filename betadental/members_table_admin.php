...
if($_POST['checksubmit'] == 'submitted')
{
backup_tables('localhost','root','QwertY4321','beta_dental');

foreach($_POST['delete_id'] as $delid) {
            //echo $delid; 
			$sql = "DELETE FROM xml_post WHERE id = $delid";
            $result = mysql_query($sql) or die(mysql_error);
    }
$item_count = count($_POST['delete_id']);
 
echo "$item_count record(s) have been deleted.";
...
<form id="admin_table_changes" name="admin_table_changes" method="POST" action="members_table_admin.php">
<fieldset>
...
?>
<tr>
<td><input type="checkbox" name="delete_id[]" value="<?php echo $member['id']; ?>"></td>
...
var cc=$('input[name="delete_id[]"]:checked').length;
var r=confirm('Are you sure you wish to delete '+cc+' Record(s)? \n\n This action cannot be undone...');
if (r==true)
  {
  document.getElementById("admin_table_changes").submit();
  }
else
  {
  }
...
<br>
<input type="text" name="checksubmit" value="submitted" style="display: none;"/>
<input type="button" name="SubmitDelete" value="Delete Selected Records" onclick="countchecked();">
